...
def run_medsam_inference_batched(
    ...
    use_amp=False, bbox_prompt_type="mask"
):
    ...
    for start_idx in range(0, depth, batch_size):
        ...
        for idx in range(start_idx, end_idx):
            if bbox_prompt_type == "whole":
                bbox = np.array([0, 0, W-1, H-1], dtype=np.float32)
                if not np.any(mask_vol[idx]):  # Still skip empty mask slices
                    preds[idx] = 0
                    continue
            else:
            ...
            batch_raw_slices.append(raw_vol[idx])

        if not batch_raw_slices:
            continue

        batch_tensor = torch.stack([preprocess_slice(s) for s in batch_raw_slices]).to(device)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            for bbox in batch_bboxes:
                ...
                scaled_box = np.array([
                    xmin / W * medsam_input_size,
                    ymin / H * medsam_input_size,
                    xmax / W * medsam_input_size,
                    ymax / H * medsam_input_size
                ], dtype=np.float32)
        ...
        for i in range(low_res_logits.shape[0]):
            ...
            pred_prob = torch.sigmoid(F.interpolate(logit, size=(H, W), mode='bicubic', align_corners=False))
            pred_mask = (pred_prob.squeeze().cpu().numpy() > 0.5).astype(np.uint8)
            preds[batch_valid_indices[i]] = pred_mask

        del batch_tensor, emb, sparse_emb, dense_emb, low_res_logits, pred_prob
        torch.cuda.empty_cache()
        gc.collect()

        print(f"Processed slices {start_idx} to {end_idx-1}")

    save_path = os.path.join(save_dir, os.path.basename(raw_tiff_path).replace("data.tif", "pred.tif"))
    tifffile.imwrite(save_path, preds, compression='zlib', imagej=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--raw_tiff_path", default="/results/raw")
    parser.add_argument("--mask_tiff_path", default="/results/mask")
    parser.add_argument("--pred_save_dir", default="/results/pred")
    parser.add_argument("--medsam_checkpoint", default="/data/medsam_best.pth")
    ...
    # If either input is a directory, process all pairs
